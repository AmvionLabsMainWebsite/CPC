...
function ProductSub() {
    ...
    const selectedCategoryItems = data
        ? data.find((c) => c.categoryURL === category)?.items || []
        : [];

    const headingSection = data
        ? data.find((c) => c.categoryURL === category)?.subsection || []
        : [];
    ...
    const final = sectionCategoryItems.length > 0
    ...
    useEffect(() => {
        if (section) {
            setSelectSectionCategory(section.toLowerCase());
        }
    }, [section]);
    ...
    const h1Map = {
        "biochemistry-analyzer": {
            "fully-automatic": "Fully Automatic Biochemistry Analyzer",
            "semi-automatic": "Semi Automatic Biochemistry Analyzer",
            "reagents": "Biochemistry Reagents for Clinical Chemistry",
        },
        "electrolyte-analyzer": {
            "with-auto-loader": "Electrolyte Analyzer with Auto Loader",
            "without-auto-loader": "Electrolyte Analyzer without Auto Loader",
        },
        "hematology-analyzer": {
            "3-part": "3 Part Hematology Analyzer for Diagnostic Labs",
            "5-part": "5 Part Hematology Analyzer for Clinical Use",
            "esr-analyzer": "Automatic ESR Analyzer for Clinical Laboratories",
        },
        "immunoassay-analyzer": {
            "elisa": "Fully Automated ELISA Immunoassay Analyzer",
            "immunoblot": "Immunoblot Analyzer and Test System",
            "ifa": "Automated Immunofluorescence (IFA) Analyzer",
            "clia": "Chemiluminescence Immunoassay (CLIA) Analyzer",
            "trace": "Trace Immunoassay and Prenatal Analyzer",
            "fia": "Fluorescence Immunoassay (FIA) Analyzer",
        },
    ...
    const h1Text =
        h1Map[category]?.[section] || categoryToShow;
